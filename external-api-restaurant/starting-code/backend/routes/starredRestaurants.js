...
router.get("/", async (_req, res) => {
  const { data, error } = await supabaseProvider.from("food_starred").select(`
		id,
		comment,
		foods (
			id,
...
router.get("/:id", async (req, res) => {
  ...
  const { data } = await supabaseProvider.from("food_starred").select("*").eq("id", id);
...
router.post("/", async (req, res) => {
  ...
  // Find the restaurant with the matching id.
  const { data: restaurantData } = await supabaseProvider.from("foods")
    .select("*")
    .eq("id", id);
  ...
  const { data, error } = await supabaseProvider.from("food_starred").insert([
    { food_id: id, comment: null }]).select(`
		id,
		comment,
		foods (
			id,
...
router.delete("/:id", async (req, res) => {
  ...
  const { error } = await supabaseProvider.from("food_starred")
    .delete()
    .match({ id: id });
